using System;

namespace Task_DEV_3
{
    /// <summary>
    /// The mmain class of program.
    /// </summary>
    class EntryPoint
    {
        /// <summary>
        /// The entry point of program.
        /// </summary>
        /// <param name="args">args[0] is cost, args[1] is perfomance, args[2] is criterion</param>
        static void Main(string[] args)
        {
            try
            {
                if(args.Length < 3)
                {
                    throw new FormatException("there is not enough elements.");
                }
                Company epam = new Company();
                switch(Convert.ToInt32(args[2]))
                {
                    case 1:
                        CreaterTeam firstCriterion = new CreaterTeamByTheFirstCriterion();
                        epam.GetEmployees(firstCriterion, Convert.ToInt32(args[0]));
                        return;
                    case 2:
                        CreaterTeam secondCriterion = new CreaterTeamByTheSecondCriterion();
                        epam.GetEmployees(secondCriterion, Convert.ToInt32(args[1]));
                        return;
                    case 3:
                        CreaterTeam thirdCriterion = new CreaterTeamByTheThirdCriterion();
                        epam.GetEmployees(thirdCriterion, Convert.ToInt32(args[1]));
                        return;
                    default:
                        Console.WriteLine("there is not such criterion.");
                        return;
                }
            }
            catch(Exception e)
            {
                Console.WriteLine($"Error: {e.Message}");
            }
        }
    }
}
